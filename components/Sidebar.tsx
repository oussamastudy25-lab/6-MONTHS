  ...
  return (
    <nav className="w-[196px] min-w-[196px] bg-[#0A0A0A] flex flex-col border-r border-[#1E1E1E]">
      <div className="px-4 py-[18px] border-b border-[#1E1E1E]">
        ...
        <div className="text-[9px] text-[#888] tracking-[.15em] uppercase mt-1">Habit OS · 2026</div>
      </div>
        ...
        {NAV.map((item, i) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-2.5 px-4 py-2.5 text-[10.5px] font-semibold tracking-[.1em] uppercase border-l-2 transition-all ${active ? 'text-[#FF5C00] border-[#FF5C00] bg-[rgba(255,92,0,.07)]' : 'text-[#888] border-transparent hover:text-white hover:bg-white/[.04]'}`}
            >
