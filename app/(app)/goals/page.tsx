...
export default function GoalsPage() {
  ...
  return (
    <>
        ...
        <div className="ml-auto flex gap-2">
          {archived.length>0 && (
            <button onClick={() => setShowArchived(s=>!s)}
              className={`px-3 py-1.5 rounded border text-[10px] font-bold uppercase tracking-[.1em] transition-all ${showArchived?'bg-[#0A0A0A] text-white border-[#0A0A0A]':'border-[#dedede] text-[#888] hover:border-[#0A0A0A] hover:text-[#0A0A0A]'}`}>
      ...
      {/* Form */}
      {showForm && (
        <div className="px-6 py-4 border-b border-[#efefef] bg-[#fafafa] flex-shrink-0">
          <div className="max-w-2xl grid grid-cols-2 gap-3">
            <input className="col-span-2 bg-white border border-[#dedede] rounded-md px-3 py-2 text-[13px] outline-none focus:border-[#FF5C00]"
            ...
            <div>
              ...
              <select className="w-full bg-white border border-[#dedede] rounded-md px-3 py-2 text-[13px] outline-none focus:border-[#FF5C00]"
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="date" className="w-full bg-white border border-[#dedede] rounded-md px-3 py-2 text-[12px] outline-none focus:border-[#FF5C00]"
            ...
            <div className="col-span-2 flex gap-2">
              ...
              <button onClick={()=>{setShowForm(false);setEditId(null)}} className="border border-[#dedede] text-[10px] font-bold uppercase tracking-[.1em] px-4 py-2 rounded hover:border-[#0A0A0A] transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        <div className="space-y-3 max-w-4xl">
          {(showArchived ? [...active,...archived] : active).map(g => {
            ...
            return (
              <div key={g.id} className={`bg-white border rounded-lg overflow-hidden transition-all ${g.archived?'border-[#f0f0f0] opacity-70':'border-[#efefef] hover:border-[#dedede]'}`}>
                {/* Goal header */}
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className={`text-[14px] font-bold ${g.archived?'line-through text-[#888]':''}`}>{g.title}</span>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] text-[#888]">{doneMilestones}/{ms.length} done</span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] text-[#888] font-mono">{timePct}% time</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => toggleExpand(g.id)}
                        className="w-7 h-7 border border-[#dedede] rounded flex items-center justify-center text-[11px] text-[#888] hover:border-[#0A0A0A] hover:text-[#0A0A0A] transition-colors"
                        title={isExpanded?'Collapse milestones':'Show milestones'}>
                        {isExpanded ? '▲' : '▼'}
                      </button>
                      {!g.archived && (
                        <button onClick={() => startEdit(g)}
                          className="w-7 h-7 border border-[#dedede] rounded flex items-center justify-center text-[11px] text-[#888] hover:border-[#0A0A0A] hover:text-[#0A0A0A] transition-colors">✎</button>
                      )}
                      {!g.archived ? (
                        <button onClick={() => archiveGoal(g.id, true)}
                          className="w-7 h-7 border border-[#dedede] rounded flex items-center justify-center text-[11px] text-[#888] hover:border-[#22c55e] hover:text-[#22c55e] transition-colors"
                          title="Archive (mark as accomplished)">✓</button>
                      ) : (
                        <button onClick={() => archiveGoal(g.id, false)}
                          className="w-7 h-7 border border-[#dedede] rounded flex items-center justify-center text-[9px] text-[#888] hover:border-[#FF5C00] hover:text-[#FF5C00] transition-colors"
                          title="Unarchive">↩</button>
                      )}
                      <button onClick={() => deleteGoal(g.id)}
                        className="w-7 h-7 border border-[#dedede] rounded flex items-center justify-center text-[13px] text-[#888] hover:bg-[#FBE9E7] hover:border-[#e0a0a0] hover:text-[#8B0000] transition-colors">×</button>
                    </div>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 bg-[#fafafa] border-t border-[#f7f7f7]">
                    <div className="pt-3">
                      {ms.map((m,i) => (
