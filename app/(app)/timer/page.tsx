...
export default function TimerPage() {
  ...
  async function startTimer() {
    ...
    const { data } = await sb.from('focus_sessions').insert({
      user_id: user.id, category_id: activeCat, date: today,
      started_at: now, duration_minutes: 0, note
    }).select().single()
    if (data) { setRunning(data); setElapsed(0); setSessions(prev => [...prev, data]) }
  }

  async function stopTimer() {
    if (!running) return
    const now = new Date().toISOString()
    const base = running.duration_minutes
    ...
    setRunning(null); setElapsed(0); setNote('')
    load()
  }
  ...
  // Stats helpers
  function todayMins(catId: string) {
    return sessions.filter(s => s.category_id===catId && s.ended_at).reduce((a,s) => a+s.duration_minutes, 0)
  }
  ...
  const pct = currentCat ? Math.min(100, Math.round(todayMins(activeCat!) / currentCat.target_minutes * 100)) : 0
  const secs = running && running.category_id === activeCat ? elapsed : 0
      ...
      <div className="bg-white px-6 py-3 border-b-2 border-[#0A0A0A] flex items-center flex-shrink-0">
        <div>
          ...
          <div className="text-[10px] text-[#888] tracking-[.12em] uppercase mt-0.5">
            Total today: {fmtMins(todayTotal)}
          </div>
        </div>
            ...
            <div className="p-6">
              {categories.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <div className="text-[13px] text-[#888] mb-5">Create focus categories — Study, Content, Business, anything you want to track time for.</div>
                  <button onClick={() => setShowSetup(true)} className="bg-[#FF5C00] text-white text-[10px] font-bold uppercase tracking-[.1em] px-4 py-2 rounded-md hover:bg-[#FF7A2E] transition-colors">
                    + Create Category
                  </button>
                </div>
              ) : (
                  ...
                  {currentCat && (
                    <>
                      ...
                      <div className="text-center mb-6">
                        <div className="font-mono text-[72px] font-bold leading-none mb-2" style={{color: running && running.category_id===activeCat ? currentCat.color : '#0A0A0A'}}>
                          {fmtTime(secs)}
                        </div>
                        ...
                        <div className="flex items-center gap-3 justify-center mb-4">
                          <div className="w-48 h-2 bg-[#efefef] rounded-full overflow-hidden">
                            <div className="h-full rounded-full transition-all" style={{width:`${pct}%`, background:currentCat.color}} />
                          </div>
                          <span className="text-[11px] text-[#888] font-mono">{fmtMins(todayMins(activeCat!))} / {fmtMins(currentCat.target_minutes)} target</span>
                        </div>
                        ...
                        {/* Start / Stop */}
                        {running && running.category_id === activeCat ? (
                      ...
                      {sessions.filter(s => s.category_id===activeCat).length > 0 && (
                        <div>
                          <div className="text-[9px] font-bold text-[#bcbcbc] tracking-[.16em] uppercase mb-2">Today's Sessions</div>
                              ...
                              return (
                                <div key={s.id} className="flex items-center gap-3 px-4 py-3 bg-white border border-[#efefef] rounded-lg hover:border-[#dedede] group transition-colors">
                                  ...
                                  <div className="flex-1">
                                    {s.note && <div className="text-[11px] font-semibold text-[#0A0A0A]">{s.note}</div>}
                                  ...
                                  <button onClick={() => deleteSession(s.id)}
                                    className="text-[11px] text-[#bcbcbc] hover:text-[#ef4444] opacity-0 group-hover:opacity-100 transition-all">×</button>
                                </div>
                              )
                            })}
                          </div>
                  ...
                  <div key={c.id}>
                    ...
                    <div className="bg-white border border-[#efefef] rounded-lg p-4">
                          ...
                          return (
                            <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group relative">
                              ...
                              <div className="w-full rounded-sm" style={{
                                height:`${h}%`,
                                background: d.mins > 0 ? c.color : '#efefef',
                      ...
                      <div className="flex items-center justify-between text-[10px] text-[#888]">
                        <span>7-day total: <span className="font-bold font-mono" style={{color:c.color}}>{fmtMins(total7)}</span></span>
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-[#bcbcbc]">= {fmtMins(form.target_minutes)}</span>
                </div>
              </div>
              <div>
              ...
              <button onClick={saveCategory}
                className="w-full bg-[#FF5C00] text-white text-[10px] font-bold uppercase tracking-[.1em] py-2 rounded-md hover:bg-[#FF7A2E] transition-colors">
