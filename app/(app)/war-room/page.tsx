      ...
      <div className="flex flex-col h-full">
        ...
        <div className="bg-[#0A0A0A] px-6 py-3 flex items-center gap-4 flex-shrink-0 border-b border-[#1E1E1E]">
          <div>
            ...
            <div className="text-[10px] text-[#555] tracking-[.15em] uppercase">{new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</div>
          </div>
          {examMode&&(
            <div className="px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-[.1em]" style={{background:'rgba(255,92,0,.15)',color:'#FF5C00',border:'1px solid rgba(255,92,0,.3)'}}>
              ⚡ EXAM MODE — {examName}
            </div>
          )}
          {score!==null&&(
          ...
          <div className="p-4 grid gap-4 max-w-5xl" style={{gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))'}}>
            ...
            <div className="bg-white border-2 border-[#0A0A0A] rounded-lg overflow-hidden">
              <div className="bg-[#0A0A0A] px-4 py-3 flex items-center justify-between">
                ...
                <span className="font-mono text-[13px] font-bold" style={{color:nnDone===nns.length&&nns.length>0?'#22c55e':'#FF5C00'}}>{nnDone}/{nns.length}</span>
              </div>
              {nns.length===0?(
                <div className="p-4 text-[12px] text-[#888]">Add non-negotiables in Setup →</div>
              ):nns.map(nn=>(
                <button key={nn.id} onClick={()=>toggleNN(nn)} className="w-full flex items-center gap-3 px-4 py-3 border-b border-[#f7f7f7] last:border-0 hover:bg-[#fafafa] transition-colors text-left">
                  ...
                  <div key={bh.id} className="border-b border-[#f7f7f7] last:border-0">
                    <div className="px-4 py-4 flex items-center gap-3">
                      <div className="flex-1">
                        ...
                        <div className="text-[10px] text-[#888] mt-0.5">{bh.lastRelapse?`Last relapse: ${bh.lastRelapse}`:'No relapses logged'}</div>
                      </div>
                    ...
                    <div className="px-4 pb-3 flex gap-1.5 flex-wrap">
                      {MILESTONES.map(m=>(
                        <div key={m} className={`px-2 py-0.5 rounded text-[9px] font-bold ${bh.cleanDays>=m?'bg-[#FF5C00] text-white':'bg-[#f7f7f7] text-[#bcbcbc]'}`}>{m}d</div>
                      ))}
                    </div>
                    {showRelapseFor===bh.id?(
                      <div className="px-4 pb-4 space-y-2">
                        ...
                        <div className="flex gap-2 pt-1">
                          <button onClick={()=>logRelapse(bh.id)} className="flex-1 bg-[#ef4444] text-white text-[9px] font-bold uppercase tracking-[.1em] py-2 rounded hover:bg-red-600 transition-colors">Confirm Relapse</button>
                          <button onClick={()=>{setShowRelapseFor(null);setRelapseForm({time_of_day:'',trigger_cause:'',prior_activity:'',note:''})}} className="px-3 text-[9px] text-[#888]">Cancel</button>
                        </div>
                      </div>
                    ):(
                      <div className="px-4 pb-3">
                        <button onClick={()=>setShowRelapseFor(bh.id)} className="text-[9px] font-bold uppercase tracking-[.1em] text-[#bcbcbc] hover:text-[#ef4444] transition-colors">Log relapse →</button>
                      </div>
                    )}
                  </div>
                ))}
            ...
            {goals.length>0&&(
              <div className="bg-white border border-[#efefef] rounded-lg overflow-hidden">
                <div className="bg-[#0A0A0A] px-4 py-3"><span className="text-[11px] font-bold tracking-[.1em] text-white uppercase">Goals</span></div>
                {goals.map(g=>{
            ...
            {metrics.length>0&&(
              <div className="bg-white border border-[#efefef] rounded-lg overflow-hidden">
                <div className="bg-[#0A0A0A] px-4 py-3"><span className="text-[11px] font-bold tracking-[.1em] text-white uppercase">Weekly Numbers</span></div>
                {metrics.map(m=>{
                    ...
                    <div key={m.id} className="px-4 py-3 border-b border-[#f7f7f7] last:border-0 flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        <input type="number" className="w-20 bg-[#f7f7f7] border border-[#dedede] rounded px-2 py-1.5 text-right font-mono text-[13px] font-bold outline-none focus:border-[#FF5C00]"
                          placeholder="0" value={m.value??''} onChange={e=>updateMetric(m.id,e.target.value)}/>
            ...
            {nns.length===0&&badHabits.length===0&&goals.length===0&&(
              <div className="col-span-full text-center py-16">
                ...
                <div className="text-[13px] text-[#888]">Go to Setup to configure your non-negotiables, bad habits, and metrics.</div>
              </div>
            )}
          </div>
        </div>
